...
class Tarot:
    def __init__(self,):
        ...
        self.played_taker_chien: List[int] = []
    ...
    def apply_action(self, action: int) -> None:
        if self.phase == Phase.BIDDING:
            ...
            Action.apply_chien_action(
                self.hands[self.current], self.chien, self.discard, action)
            if len(self.discard) == Const.CHIEN_SIZE:
                self.phase = Phase.DECLARE
                self.update_played_taker_chien()
        ...
        if self.phase == Phase.BIDDING:
            if len(self.bids) >= Const.NUM_PLAYERS:
                ...
                if self.taker_bid < Const.BID_GARDE_SANS:
                    ...
                    self.played_taker_chien = self.chien.copy()
            ...
            if len(self.trick) < Const.NUM_PLAYERS:
                self.next_player()
            else:
    ...
    def update_played_taker_chien(self) -> None:
        self.played_taker_chien = self.chien + self.discard
        chien_know_cards = set()
        for card in self.played_taker_chien:
            if card not in chien_know_cards and card not in self.chien:
                chien_know_cards.add(card)
        self.played_taker_chien = list(chien_know_cards)
    ...
    def tensor(self) -> List[int]:
        ...
        tensor = [*self.played_cards, *current_trick, *self.played_tricks,
