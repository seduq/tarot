...
class TarotISMCTSNode:
    ...
    def select_action(self, legal_actions: List[int]) -> int:
        ...
        return max(valid_actions, key=lambda a: self.ucb_score(a, self.exploration_constant))
...
class TarotISMCTSAgent:
    ...
    def __init__(self,
        ...
        self.pw_alpha = pw_alpha      # Exponent: lower values = more conservative expansion
        self.pw_constant = pw_constant  # Multiplier: higher values = more actions expanded
        ...
        while not game.is_terminal():
            ...
            if info not in self.tree:
                self.tree[info] = TarotISMCTSNode(
                    info, exploration_constant=self.exploration_constant)
    ...
    def determinization(self, game: Tarot) -> Tarot:
        ...
        # Cards known to the taker from the chien (if any)
        taker_know_cards = game.taker_chien_hand if game.taker_chien_hand else []
        ...
        for player in range(Const.NUM_PLAYERS):
            if player != self.player:
                ...
                # If this player is the taker, add any known chien cards first
                if determinized.taker == player and determinized.taker_chien_hand:
                    determinized.hands[player] = determinized.taker_chien_hand.copy(
                    )
                    # Reduce the number of random cards needed
                    original_hand_size -= len(determinized.taker_chien_hand)
